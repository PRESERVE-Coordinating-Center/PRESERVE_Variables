...
select * from concept where concept_code = '45816000' and vocabulary_id = 'SNOMED';
...
where ancestor_concept_id in (198199) -- pyelo SNOMED code
...
from concept where concept_id in (
	...
	where relationship_id = 'Maps to'
		...
		select descendant_concept_id
		from concept_ancestor
		where ancestor_concept_id in (198199)
))
and vocabulary_id in ('ICD9CM', 'ICD10', 'ICD10CM', 'SNOMED')
union
...
from concept
where (lower(concept_name) like '%pyelonephritis%')
