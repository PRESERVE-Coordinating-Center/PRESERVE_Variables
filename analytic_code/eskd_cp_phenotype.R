...
#' Identify cohort with End-Stage Kidney Disease (ESKD)  
#' The computable phenotype was developed to query patients ESKD patients (patients who had either undergone kidney chronic dialysis or kidney transplant) 
#' and was validated against USRDS dataset and a subset of chart review patients. 
#' The most optimial computable phenotype for identifying ESKD required one code from Group A (more specific) 
#' or two distinct codes from Group B (more sensitive) and at least five eGFR measurements ≤15 mL/min/1.73m² on different days. 
#' This algorithm achieved an F1 score of 0.92 (95% CI 0.88-0.95), sensitivity of 94.6% (95% CI 92.8%-96.2%), and PPV of 86.8.7% (95% CI 84.4%-89.3%)
...
#' @param eskd_codeset Validated ESKD codeset 
...
#' 
get_eskd_cohort <- function(cohort_tbl, 
                            ...
                            min_age,
                            max_age,
                            transplant_codeset = load_codeset("eskd_cp_USRDS"),
                            num_egfr = 5,
                            max_egfr_val = 15){

    # patients with any eskd condition & procedure codes within the desired time
    cohort_tbl <- cohort_tbl %>%
     ...
     cohort_first <- cohort_tbl %>%
        group_by(patid) %>%
        slice_min(report_date, n = 1, with_ties = FALSE) %>%
        ungroup() %>%
        inner_join(demographic_tbl %>%
                   select(patid, birth_date), by = "patid") %>%
        mutate(age_at_ced = as.numeric(difftime(report_date, birth_date, units = "days")) / 365.25) %>%
    ...
    group_B <- cohort_tbl %>%
               filter(group == "B") %>%
               group_by(patid) %>%
               summarise(n_px = n_distinct(report_date)) %>% ungroup() %>%
               filter(n_px >= 2) %>%
...
#' Get first date of transplant or post-transplant complication procedures 
#'
...
#'
get_dx_px <- function(cohort = cdm_tbl("demographic"),
                      procedure_table = cdm_tbl("procedures"),
                      condition_table = cdm_tbl("condition"),
                      dx_codeset, px_codeset) {
  tbl <- get_dx(cohort, 
        dx_codeset = dx_codeset) %>% 
        mutate(type = "condition") %>%
        select(patid, id = conditionid, encounterid, report_date = report_date, vocabulary_type = condition_type, site, concept_code = condition, type) %>% 
        union(get_px(cohort,
                  px_codeset = px_codeset) %>%
        mutate(type = "procedure") %>% 
        select(patid, id = proceduresid, encounterid, report_date = admit_date, vocabulary_type = px_type, site, concept_code = px, type), by = c("patid"))# 
